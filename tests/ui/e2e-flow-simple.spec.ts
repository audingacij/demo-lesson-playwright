...
test('login and verify order can not be created with empty Name field', async ({ page }) => {
  ...
  await expect(page.getByText('The field must be filled in.'),
  ).toBeVisible();
})

test('login and verify order can not be created with one symbol in the Name field', async ({ page }) => {
  const authPage = new LoginPage(page)
  await authPage.open()
  const orderCreationPage = await authPage.signIn(USERNAME, PASSWORD)
  await orderCreationPage.userNameField.fill(faker.lorem.word(1))
  await orderCreationPage.phoneNumber.fill(faker.lorem.word(6))
  await expect(orderCreationPage.orderButton).toBeDisabled()
  await expect(page.getByText('The field must contain at least of characters: 2'),
).toBeVisible();
})

test('login and verify order can not be created with 5 symbols in the Phone field', async ({ page }) => {
  const authPage = new LoginPage(page)
  await authPage.open()
  const orderCreationPage = await authPage.signIn(USERNAME, PASSWORD)
  await orderCreationPage.userNameField.fill(faker.lorem.word(2))
  ...
  await expect(orderCreationPage.orderButton).toBeDisabled()
  await expect(page.getByText('The field must contain at least of characters: 6'),
  ).toBeVisible();
})